...
class Todo:
    def __init__(self, task, category, monitors, server, time,
        ...
        self.server = server
        self.time = time
        self.date_added = date_added if date_added is not None else datetime.datetime.now().timestamp()
    ...
    def __repr__(self) -> str:
        return f"({self.task}, {self.category}, {self.monitors}. {self.server}, {self.time}, {self.date_added}, {self.date_completed}, {self.status}, {self.position})"
